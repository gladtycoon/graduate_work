...
class BorrowRecord(models.Model):
    ...
    )  # когда вернули (если вернули)
    status = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="BORROWED"
    )  # текущий статус книги
    notes = models.TextField(blank=True)  # примечания

    def __str__(self):
        return f"{self.user.email} - {self.book.title}"  # отображение в админке (переопределено в borrow/admin.py)
