...
class CustomUserAdmin(UserAdmin):
    list_display = ('id', 'email', 'username', 'is_librarian', 'is_staff', 'is_active')       # какие поля показывать в списке всех записей
    ...
    fieldsets = UserAdmin.fieldsets + (                                                       # группировка полей на странице редактирования
        ('Дополнительные поля', {                                                             # (разбивает длинную форму на логические блоки с заголовками)
